...
def _default_session_state() -> Dict[str, Optional[str]]:
    return {
        "active_dataset": None,
...
def reset_processing_state() -> Dict[str, Optional[str]]:
...
def reset_session_state() -> Dict[str, Optional[str]]:
    _SESSION_STATE["active_dataset"] = None
    return reset_processing_state()


def set_active_dataset(dataset_id: Optional[str]) -> Dict[str, Optional[str]]:
...
def set_pipeline_status(status: str) -> Dict[str, Optional[str]]:
...
def set_anomaly_status(status: str) -> Dict[str, Optional[str]]:
...
def set_prediction_status(status: str) -> Dict[str, Optional[str]]:
    if status not in MODEL_STATUS_ALLOWED:
